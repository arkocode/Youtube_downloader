...
def get_links(message):
    try:
        ...
        resolutions_label = ""
        resolutions = ["","","","","",""]
        audio_size = audio_highq_size
        i = 0
        for key in streams:
            if key == '144p' or key == '240p':
                audio_size = audio_lowq_size
            elif key == '360p' or key == '480p':
                audio_size = audio_medq_size
            elif key == '720p' or key == '1080p':
                audio_size = audio_highq_size
            resolutions[i] = key
            i += 1
